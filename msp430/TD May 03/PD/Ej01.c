...
void LedsSequencer_A(){
  ...
  switch (presentState){
      case S0:
        P1OUT=(P1OUT | BIT6) & ~(BIT0+BIT1);     //Red1 ON; Green1 OFF 
        if (endedCountFlag==SET){
          endedCountFlag=CLEAR;
          pulseCount=1;                   //time for Green1 
          presentState=S1;                //in S1 Green1 on
        }        
        break;
         
       case S1:
        P1OUT=(P1OUT & ~(BIT6+BIT0)) | BIT1;     //Red1 OFF; Green1 ON    
        if (endedCountFlag==SET){
          endedCountFlag=CLEAR;
          pulseCount=3;                   //time for Red1 
          presentState=S2;                //in S0 Red1 is on
        }  
        break;

       case S2:
        P1OUT=(P1OUT | BIT0) & ~(BIT1+BIT6);     //Red1 OFF; Green1 ON    
        if (endedCountFlag==SET){
          endedCountFlag=CLEAR;
          pulseCount=1;                   //time for Red1 
          presentState=S3;                //in S0 Red1 is on
        }  
        break;
...
switch (presentState){
      case S0:
        P2OUT=(P2OUT | BIT6) & ~(BIT0+BIT1);     //Red2 ON; Green2 OFF 
        if (elapsedTimeFlag==SET){
          elapsedTimeFlag=CLEAR;
          timeCount=100;                   //time for Green2 
          presentState=S1;                //in S1 Green2 on
        }        
        break;
         
       case S1:
        P2OUT=(P2OUT & ~(BIT6+BIT0)) | BIT1;     //Red2 OFF; Green2 ON    
        if (elapsedTimeFlag==SET){
          elapsedTimeFlag=CLEAR;
          timeCount=300;                   //time for Red2 
          presentState=S2;                //in S0 Red2 is on
        }  
        break;
		
	  case S2:
        P2OUT=(P2OUT | BIT0) & ~(BIT1+BIT6);     //Red2 ON; Green2 OFF 
        if (elapsedTimeFlag==SET){
          elapsedTimeFlag=CLEAR;
          timeCount=100;                   //time for Green2 
          presentState=S3;                //in S1 Green2 on
        }        
        break;  
        ...
        if (elapsedTimeFlag==SET){
          elapsedTimeFlag=CLEAR;
          timeCount=500;                   //time for Green2 
          presentState=S0;                //in S1 Green2 on
        }        
        break;
    }
